...
- [x] **Consolidate `build_img_tag` in social.rb**: Removed hand-rolled `build_img_tag` — now uses the shared `html_attrs` helper.
- [x] **Consolidate `build_img_tag` in carousel_image.rb**: Removed hand-rolled `build_img_tag` — now uses the shared `html_attrs` helper.
